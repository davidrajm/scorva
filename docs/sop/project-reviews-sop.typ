...
#set document(
  title: "Project Reviews — Standard Operating Procedure",
  author: "Project Reviews Plugin",
)
...
#set page(
  ...
  header: context {
    if counter(page).get().first() > 1 [
      ...
      #grid(
        columns: (1fr, 1fr),
        align(left)[Project Reviews SOP],
...
This Standard Operating Procedure (SOP) describes how to use the *Project Reviews* WordPress plugin from first login through project setup, marking, reporting, and project closure. It is written for two primary audiences:
...
#tip[
  *Quick navigation:* Part I covers coordinators; Part II covers reviewers. Shared concepts (login, terminology, troubleshooting) appear first. Figures marked with live screenshots use captures from `npm run sop:screenshots` (folder `screenshots/2026-05-22_012914/`); remaining figures show placeholders until captured manually.
]
...
== What Project Reviews replaces

Project Reviews replaces spreadsheet-and-email marking workflows with a governed pipeline:
...
#v(6pt)
#url-box("Reviewer marking workspace", "https://your-site.edu/reviews/mark/")

#screenshot(
  "01-landing-login",
  "Landing page shown to unauthenticated visitors at `/reviews/`.",
  "Open `/reviews/` in a private/incognito browser window before logging in. Capture the full page with the Project Reviews wordmark and Log in button.",
)

== Signing in

#procedure("Log in to Project Reviews", "both")[
  ...
  + After login, you are routed automatically:
    - *Coordinators* → Coordinator dashboard at `/reviews/`
    - *Reviewers only* → Marking workspace at `/reviews/mark/`
    - *Both roles* → Coordinator dashboard (use the top navigation to switch to Marking)
]
...
#warning[
  Reviewers receive *provisioned WordPress accounts* created by coordinators. If you cannot log in, contact your project coordinator — do not share credentials.
]
...
- *Closed* — marking stopped; reviewer accounts may be disabled
...
#table(
  ...
  [3], [Reviewers], [Add reviewers; provision WordPress accounts; bulk CSV import supported.],
...
#procedure("Add and provision reviewers", "coordinator")[
  + Add reviewers individually (name + email) or via *Import reviewers* CSV; assign each to a panel.
  + For each new reviewer row, click *Send credentials* and wait for *Account linked* (links the WordPress user — required before they see assignments).
  + Use *Resend credentials* if the invite email was not received.
  + Optionally link an existing WordPress user via *Link WordPress user* when editing a row.
  + Click *Continue to Reviews & rubrics* (or open that wizard tab).
]

#screenshot(
  "10-wizard-reviewers",
  "Reviewers step with roster and provisioning status.",
  "Reviewers step showing reviewer names, emails, provision/link status, and import button.",
)
...
#procedure("Open marking", "coordinator")[
  ...
  + Reviewers see assignments in their marking workspace.
]
...
#procedure("Chase incomplete marking", "coordinator")[
  ...
  + Contact reviewers directly (outside the plugin) or use notification emails if configured.
...
#warning[
  Closing a project stops marking and may disable provisioned reviewer accounts. Export all committee deliverables *before* closing.
]
...
#part-page(
  "Part II",
  "Reviewer procedures — assignments, marking, freeze, panel head tasks",
  pr-reviewer,
)
...
Reviewers work in a focused three-step funnel:
...
#grid(
  ...
  #block(stroke: 0.75pt + pr-border, radius: 6pt, inset: 10pt, fill: pr-reviewer.lighten(92%))[
    #align(center)[*1. Assignments* \
    Pick project / review / panel]
  ],
  [→],
  #block(stroke: 0.75pt + pr-border, radius: 6pt, inset: 10pt, fill: pr-reviewer.lighten(92%))[
    #align(center)[*2. Marking grid* \
    All students in panel]
  ],
  [→],
  #block(stroke: 0.75pt + pr-border, radius: 6pt, inset: 10pt, fill: pr-reviewer.lighten(92%))[
    #align(center)[*3. Rubric form* \
...
#url-box("Your marking workspace", "https://your-site.edu/reviews/mark/")

= Your assignments

After login, the *Your assignments* page lists available and blocked assignments.
...
#tip[
  *Draft* marks can be edited until you freeze scores or the coordinator locks the review. If you see *No assignments yet*, ask the coordinator to run *Send credentials* for your email on the Reviewers wizard step.
]
...
#table(
  ...
  [Cannot see any assignments], [Reviewer not on panel, *Account linked* missing, or setup incomplete], [Coordinator],
  ...
  [Cannot log in], [Account not provisioned or password expired], [Coordinator / WordPress admin],
...
#block(
  ...
  #flow-node(
    "1 · Sign in",
    detail: [Visit `/reviews/` or `/reviews/mark/` · WordPress login · Auto-routed by role],
    accent: pr-primary,
  )
  #flow-arrow()
  ...
  #grid(
    ...
    #block(width: 100%)[
      ...
      #flow-arrow()
      #flow-node("12 · Close project", detail: "Export first · Disable reviewers · Closed", accent: pr-coordinator)
    ]
    ...
    #block(width: 100%)[
      #align(center)[
        #box(
          ...
          #text(size: 8pt, weight: "bold", fill: pr-reviewer)[REVIEWER · `/reviews/mark/`]
        ]
      ]
      #v(6pt)
      #block(
      ...
      #v(6pt)
      #flow-node("A · Your assignments", detail: "Pick project · review round · panel", accent: pr-reviewer)
      #flow-arrow()
      #flow-node("B · Marking grid", detail: "All students in panel · status chips", accent: pr-reviewer)
      #flow-arrow()
      #flow-node("C · Rubric form", detail: "Attendance · criterion scores · draft or submit", accent: pr-reviewer)
      #flow-arrow()
      #flow-node("D · Freeze scores", detail: "Lock personal marks when complete", accent: pr-reviewer)
      #v(4pt)
      #block(
...
#table(
  ...
  [Live screenshots], [`docs/sop/screenshots/2026-05-22_012914/` — 19 PNGs; `sop-screenshots-dir` in `lib/theme.typ`],
  [Manual screenshots still needed], [CSV import mapper, score breakdown, unfreeze queues, marks matrix, offline PDF, audit/close, mobile grid, validation/flagged/freeze dialogs, panel head flows — see `tests/e2e/sop-screenshot-manifest.ts`],
...
Automated capture IDs embedded when PNGs exist: `01`–`05`, `07`–`15`, `18`, `20`, `25`–`26`, `28`. Other figure IDs show placeholders until you add `{id}.png` manually or extend the Playwright capture spec.

#v(2em)
#align(center)[
  #text(size: 9pt, fill: pr-muted)[
    End of document · Project Reviews Plugin SOP v1.1
  ]
]
