...
final class ReviewerProvisionService
{
    ...
    public function generate_reviewer_credentials(
        ...
        if ($email === '' && $send_email) {
            return new \WP_Error(
                'pr_reviewer_missing_email',
                __('Email is required before sending credentials.', 'project-reviews'),
                ['status' => 400]
            );
        }
    ...
    public function resend_current_credentials(int $session_id, int $reviewer_id): array|\WP_Error
    {
        ...
        if ($token === '' || $encrypted === '') {
            return new \WP_Error(
                'pr_no_credentials',
                __('No credentials have been generated for this reviewer. Use "Send credentials" first.', 'project-reviews'),
                ['status' => 400]
            );
        }
        ...
        if ($password === '') {
            return new \WP_Error(
                ...
                __('Could not read stored credentials. Regenerate credentials for this reviewer.', 'project-reviews'),
                ['status' => 500]
            );
        }
        ...
        foreach ($this->panels->list_reviewers_for_session($session_id) as $reviewer) {
            ...
            if (!($result['email_sent'] ?? false)) {
                ...
                $details[] = [
                    ...
                    'email' => $email,
                    'status' => 'failed',
                    'message' => __('Email could not be sent.', 'project-reviews'),
                ];
                continue;
            }
