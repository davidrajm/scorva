...
final class PanelUnfreezeRequestRepository
{
    ...
    public function __construct(?object $wpdb = null)
    {
        ...
        if ($wpdb === null) {
            throw new \RuntimeException('PanelUnfreezeRequestRepository requires $wpdb.');
        }

        $this->wpdb = $wpdb;
        $prefix = $this->wpdb->prefix;
        $this->table = $prefix . 'pr_panel_unfreeze_requests';
     ...
     * @return array<string, mixed>|null
     */
    public function find_pending_for_panel(int $session_id, int $review_id, int $panel_id): ?array
    {
        $sql = $this->wpdb->prepare(
            "SELECT * FROM {$this->table}
             WHERE session_id = %d AND review_id = %d AND panel_id = %d AND status = %s
    ...
    public function has_pending(int $session_id, int $review_id, int $panel_id): bool
    {
        return $this->find_pending_for_panel($session_id, $review_id, $panel_id) !== null;
    }
    ...
    public function create_pending(
        ...
        $existing = $this->find_pending_for_panel($session_id, $review_id, $panel_id);
        if ($existing !== null) {
            return $existing;
        }
    ...
    {
        $requested_by = (int) ($row['requested_by_user_id'] ?? 0);
        ...
        return [
            ...
            'panel_coordinator_name' => $this->user_display_name($requested_by),
